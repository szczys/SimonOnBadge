 ...
 * Author: mike
 *
 * Created on April 2, 2016, 4:20 PM
 */
...
volatile uint16_t Uniform   @ 0x14;     //Uniform(2 bytes) 150 Hz divider, count up for 6 sec timing
volatile uint16_t RXserial  @ 0x16;     //RXserial(2 bytes) Received serial number(binary), ready for comparison
volatile uint16_t MySerial  @ 0x18;     //MySerial(2 bytes) unit serial number copied from ROM address 0x100E
